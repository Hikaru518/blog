...
<img src="ram-screenshot-slack.png" alt="Ram Slack 截图" width="335" />

如图所示，Ram 是一个 Slack 中的 AI Agent，可以在 Slack 根据我的请求进行回答。因为整个 Slack 工作区中只有我一个人，你也可以认为 Ram 就是我的数字助手。
...
<img src="slack-thread-demo.png" alt="Slack 截图" width="540" />
...
一是出于 debug 和 evaluation 的考虑。对 agent 来说，一轮 agent loop 会产生好多个工具调用，我需要知道它每一轮的输入输出和报错。因为 debug 也是我提供内容给 coding agent 进行 debug，我还提供了以 markdown 形式下载 trace 的功能。
...
另一方面，我认为一个个人助手可以天然地管理和统一我的所有 AI 聊天场景。如果我的所有代码和聊天都通过 Ram 进行，那么他就可以存储我所有的聊天对话（OpenCode 对话，搜索对话等等），那么一个 Agent 记忆的资料和输入就有了。因此要做记忆系统，一个好的存储系统也是不可或缺的，而这个数据库存储的数据和 trace 系统存储的数据非常相似。

## Prompt 历史和 Design Doc

我在 Ram 这个开源项目中提供了非常完整的 prompt 和 design 历史，保存在 `docs/plans` 中。这里面甚至记录了我对 Ram 的设计变更，也有我和 LLM 讨论的细节。
...
3. 黄老板最近在做一个自动生成 AI 短剧的项目：[wddxh/ShortVideoDirector](https://github.com/wddxh/ShortVideoDirector)，这个项目给我带来了很多欢乐。生成的 AI 漫剧太好笑了（虽然我一直没有等来我想要的短剧续集）。但是和黄老板讨论的过程中，我又一次认识到 AI Agent 中的 domain knowledge 是不可缺少的。
